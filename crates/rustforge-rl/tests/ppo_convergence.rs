//! Slow deterministic PPO Discrete CartPole learning acceptance.

use std::sync::atomic::{AtomicU64, Ordering};
use std::sync::{Arc, Mutex};
use std::time::Instant;

use rustforge_rl::agent::{cartpole_ppo_config, PpoDiscreteTrainerAdapter};
...
const FINAL_MEAN_THRESHOLD: f64 = 100.0;
const IMPROVEMENT_THRESHOLD: f64 = 50.0;
...
fn ppo_converges_on_cartpole_seed_2026() {
    ...
    let reward_id = adapter
        ...
        .expect("PPO metadata declares reward.episode")
        .id;
    ...
    eprintln!(
        "PPO seed={SEED} episodes={EPISODES} first_100_mean={first_mean:.2} \
