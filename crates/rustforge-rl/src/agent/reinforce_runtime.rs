...
const REWARD_MOVING_AVERAGE: MetricId = MetricId::new(302);
const LOSS_POLICY: MetricId = MetricId::new(303);
const ROLLOUT_SIZE: MetricId = MetricId::new(304);
const STEPS_PER_SECOND: MetricId = MetricId::new(305);

static NEXT_RUN_ID: AtomicU64 = AtomicU64::new(0);

/// Returns the approved REINFORCE integration profile for CartPole.
pub fn cartpole_reinforce_config() -> REINFORCEConfig {
    REINFORCEConfig {
        obs_dim: 4,
        num_actions: 2,
...
pub struct ReinforceTrainerAdapter<E> {
    env: E,
...
impl<E> Trainer for ReinforceTrainerAdapter<E>
where
    ...
    <E::Act as TryFrom<usize>>::Error: Debug,
{
    ...
    fn run(self: Box<Self>, context: TrainerContext) -> Result<TrainingSummary, TrainerError> {
        ...
        let result = train_reinforce_core(
            self.env,
            self.config,
            self.episodes,
    ...
    <E::Act as TryFrom<usize>>::Error: Debug,
{
    ...
    'episodes: for episode in 0..episodes {
        ...
        for step_index in 0..max_steps_per_episode {
            let logits = agent.forward(&state_buf);
